...
import { ShieldAlert, Mail, Lock, Eye, EyeOff, ArrowRight, AlertCircle } from "lucide-react";
...
export default function AdminLoginPage() {
  ...
  async function onSubmit(e) {
    ...
    setLoading(true);
    try {
      await adminLogin({ correo, password });
      navigate("/admin");
    } catch (err) {
      ...
      <div className="hidden flex-col justify-between bg-[#153C55] p-12 text-[#FFFCF6] lg:flex">
        ...
        <div>
          <span className="inline-flex items-center gap-2 rounded-full border border-white/20 bg-white/10 px-3 py-1.5 text-xs font-semibold">
            <ShieldAlert className="size-3.5" /> Backoffice — acceso restringido
          </span>
          <h1 className="mt-6 max-w-sm font-display text-3xl font-bold leading-tight">
            Panel de administración de CheckBiz
          </h1>
          <p className="mt-4 max-w-sm text-white/70">
            Revisión de identidad, veto de cédulas y moderación. Esta sección
            es exclusiva para el equipo de CheckBiz — no es la misma cuenta
            de cliente o emprendedor.
          </p>
        </div>

        <p className="text-xs text-white/50">
          © {new Date().getFullYear()} CheckBiz. Acceso registrado y auditado.
        </p>
      </div>
      ...
      <div className="relative flex items-center justify-center px-6 py-16">
        ...
        <div className="w-full max-w-sm">
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Ingresa con tu cuenta de administrador CheckBiz.
          </p>
